...
import {
  runAllChecks, runTonerCheck, runStockCheck, runSubscriptionCheck,
  runMaintenanceCheck, runSlaCheck, runNoteReminders,
...
export function startScheduler() {
  ...
  // Every minute — personal note/to-do reminders, so date+time alerts are prompt.
  cron.schedule('* * * * *', async () => {
    try { await runNoteReminders(); }
    catch (e) { logger.error({ err: e.message }, 'note reminder check failed'); }
  });

  logger.info('Background jobs registered (daily 07:00, hourly, SLA every 15m, note reminders every minute)');
}
