...
const publicUser = (u) => ({
  ...
  departmentId: u.departmentId, department: u.department ? { id: u.department.id, name: u.department.name } : null,
  companyId: u.companyId, company: u.company ? { id: u.company.id, name: u.company.name, shortName: u.company.shortName } : null,
  allCompanies: !!u.allCompanies,
  companies: Array.isArray(u.companies) ? u.companies.map((c) => ({ id: c.id, name: c.name, shortName: c.shortName })) : [],
});
...
export const register = asyncHandler(async (req, res) => {
  const { email, password, firstName, lastName, role, departmentId, companyId, companyIds, allCompanies, jobTitle, phone } = req.body;
...
export const login = asyncHandler(async (req, res) => {
  ...
  const user = await prisma.user.findUnique({ where: { email }, include: { department: true, company: true, companies: true } });
...
export const me = asyncHandler(async (req, res) => {
  const user = await prisma.user.findUnique({
    ...
    include: { department: true, company: true, companies: true },
  });
