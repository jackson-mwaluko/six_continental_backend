...
// GET /api/printers/unregistered — PRINTER-type assets not yet linked to a Printer record
export const listUnregisteredAssets = asyncHandler(async (_req, res) => {
  const assets = await prisma.asset.findMany({
    where: { type: 'PRINTER', printer: null },
    select: { id: true, serialNumber: true, name: true, model: true, manufacturer: true, location: true },
    orderBy: { createdAt: 'desc' },
  });
  res.json({ success: true, data: assets });
});

// POST /api/printers — link a PRINTER-type asset to a new Printer record so it
// appears on the Printer & Toner page and can have toner tracked against it.
export const registerPrinter = asyncHandler(async (req, res) => {
  const { assetId, modelName, monoSupported, colorSupported, monthlyDutyCycle } = req.body;

  const asset = await prisma.asset.findUnique({ where: { id: assetId } });
  if (!asset) throw ApiError.notFound('Asset not found');
  if (asset.type !== 'PRINTER') throw ApiError.badRequest('Only assets with type PRINTER can be registered here');

  const existing = await prisma.printer.findUnique({ where: { assetId } });
  if (existing) throw ApiError.conflict('This asset is already registered as a printer');

  const printer = await prisma.printer.create({
    data: {
      assetId,
      modelName: modelName?.trim() || asset.model || asset.name,
      monoSupported: monoSupported !== false,
      colorSupported: !!colorSupported,
      monthlyDutyCycle: monthlyDutyCycle ? Number(monthlyDutyCycle) : null,
    },
    include: { asset: { select: { id: true, serialNumber: true, name: true, location: true } }, toners: true },
  });

  await logActivity({ userId: req.user.id, action: 'CREATE', entity: 'Printer', entityId: printer.id, metadata: { assetId } });
  res.status(201).json({ success: true, data: { ...printer, toners: [] } });
});
