...
export const dashboardStats = asyncHandler(async (_req, res) => {
  const [
    ...
    assetsByType, pendingRequests, recentRequests, atRiskTickets,
  ] = await Promise.all([
    ...
    // Open tickets due within the next 4 hours (or already breached) — a quick
    // "what needs attention right now" glance, distinct from the raw breach count.
    prisma.ticket.findMany({
      where: {
        status: { notIn: ['RESOLVED', 'CLOSED'] },
        resolutionDueAt: { not: null, lte: new Date(Date.now() + 4 * 60 * 60 * 1000) },
      },
      select: {
        id: true, ticketNo: true, subject: true, priority: true, resolutionDueAt: true, slaBreached: true,
        assignee: { select: { firstName: true, lastName: true } },
      },
      orderBy: { resolutionDueAt: 'asc' }, take: 5,
    }),
  ]);
  ...
  const [assignments, openTickets, resolvedTickets, requests] = await Promise.all([
    prisma.assetAssignment.findMany({
      ...
      include: {
        ...
        handover: { select: { id: true, documentNo: true, attachments: true } },
      },
